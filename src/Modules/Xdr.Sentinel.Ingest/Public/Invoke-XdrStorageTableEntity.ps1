function Invoke-XdrStorageTableEntity {
    ...
    .DESCRIPTION
        Single canonical helper for all Storage Table entity ops (gate flag +
        45 stream checkpoints). Consolidates four scattered call sites that
        previously mixed AzTable cmdlets and ad-hoc Invoke-RestMethod blocks.

        Why this helper exists:
        - AzTable 2.1.0 + Microsoft.Azure.Cosmos.Table SDK does NOT reliably
          honor MI auth from New-AzStorageContext -UseConnectedAccount, which
          caused production breakage in earlier builds.
        - Ad-hoc Invoke-RestMethod calls passed `If-Match: *` which converts
          PUT from Insert-Or-Replace to Update Entity (404 if row absent),
          breaking the first-run gate-flag write.
