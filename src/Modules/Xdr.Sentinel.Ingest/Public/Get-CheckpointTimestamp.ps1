function Get-CheckpointTimestamp {
    ...
    .NOTES
        Uses Invoke-XdrStorageTableEntity (HttpClient + MI token via
        Get-AzAccessToken -ResourceUrl https://storage.azure.com/) — the
        AzTable 2.1.0 + New-AzStorageContext path did not reliably honor MI
        auth, which caused production breakage in earlier builds.
    #>
    [CmdletBinding()]
    [OutputType([datetime])]
    param(
