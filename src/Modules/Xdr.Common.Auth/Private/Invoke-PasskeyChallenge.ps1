function Invoke-PasskeyChallenge {
    ...
    .DESCRIPTION
        ...
        Portal-agnostic: this is a pure crypto primitive. The rpId default is
        'login.microsoft.com' because that's where Entra resolves passkey assertions
        for ALL portals — the L2 portals (Defender, Purview, Intune) all delegate
        to login.microsoftonline.com / login.microsoft.com for primary auth.
