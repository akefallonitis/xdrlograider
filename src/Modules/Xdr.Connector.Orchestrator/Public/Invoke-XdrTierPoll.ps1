function Invoke-XdrTierPoll {
    ...
    .PARAMETER Config
        ...
        Required keys: DceEndpoint, DcrImmutableIdsJson, StorageAccountName,
        CheckpointTable. (DCR_IMMUTABLE_IDS_JSON is the JSON map env var
        produced by the deploy template; the per-portal poller resolves a
        per-stream DCR immutableId via Get-DcrImmutableIdForStream.)
