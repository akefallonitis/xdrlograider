function Reset-XdrPortalRate429Count {
    <#
    .SYNOPSIS
        Resets the cumulative 429 counter to zero.
    ...
    .DESCRIPTION
        ...
        The counter lives in $script:Rate429Count inside the module scope
        (defined in Invoke-MDEPortalRequest.ps1 top-level). Resetting resets
        only the current process's module instance; module reimport has the
        same effect.
