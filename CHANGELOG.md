...
- **Hosting-plan parameter is now a single canonical 3-tier enum.** The legacy `functionPlanSku` parameter (Y1 / EP1 / EP2) is dropped from `main.bicep`, `mainTemplate.json`, and `createUiDefinition.json`. Operators choose `hostingPlan` = `consumption-y1` / `flex-fc1` / `premium-ep1` — the value name itself communicates the cost-vs-security trade-off. The compiled ARM derives the underlying `Microsoft.Web/serverfarms` SKU + tier from `hostingPlan` via template variables. Two new operator-selectable controls also surface in the wizard: `restrictPublicNetwork` (default false; locks down Storage / Key Vault / App Insights for regulated deploys) and `enableKeyVaultDiagnostics` (default true; sends Key Vault audit logs to the Sentinel workspace for forensic visibility on credential access). See [docs/HOSTING-PLANS.md](docs/HOSTING-PLANS.md) and [docs/SECURITY-NOTES.md](docs/SECURITY-NOTES.md). Drift between `main.bicep` and `deploy/compiled/mainTemplate.json` is now caught by two new offline Pester gates (`tests/arm/MainTemplateBicepConsistency.Tests.ps1`, `tests/arm/CreateUiDefinitionConsistency.Tests.ps1`).
