...
#
# Runs under ~2 minutes for 45 streams. Does NOT ingest — read-only probe.
...
Import-Module "$repoRoot/src/Modules/Xdr.Common.Telemetry/Xdr.Common.Telemetry.psd1"       -Force
...
foreach ($entry in $entries) {
    ...
    try {
        # v0.1.0-beta: honor manifest Headers (with {TenantId} template resolution)
        # so XSPM / MTO / other endpoints that require x-tid, x-ms-scenario-name,
        # or mtoproxyurl headers get them. Without this, the audit tool silently
        # omits the headers and gets 400 even for correctly-configured entries.
        $extraHeaders = @{}
        if ($entry.ContainsKey('Headers') -and $entry.Headers) {
            foreach ($k in $entry.Headers.Keys) {
                $v = $entry.Headers[$k]
                if ($v -is [string] -and $v -match '\{TenantId\}') {
                    $v = $v -replace '\{TenantId\}', [string]$session.TenantId
                }
                $extraHeaders[$k] = $v
            }
        }
