# Per-Op Pester replay test · Defender/AnalyticsData/GetTopThreats
...
BeforeAll {
    ...
    $script:Manifest = Import-PowerShellDataFile -Path (Join-Path $script:RepoRoot 'manifests\Defender\AnalyticsData.psd1') -ErrorAction Stop
    $script:Op = @($script:Manifest.Operations | Where-Object { $_.OperationKey -eq 'GetTopThreats' })[0]
}

Describe 'Manifest contract · Defender/AnalyticsData/GetTopThreats' {
    It 'manifest entry exists for GetTopThreats · no IsActive flag (v11 §4.11)' {
        $script:Op | Should -Not -BeNullOrEmpty
        $script:Op.OperationKey | Should -Be 'GetTopThreats'
    ...
    It 'canonical table + stream naming' {
        $script:Op.WorkspaceTable | Should -Be 'Defender_AnalyticsData_CL'
        $script:Op.DcrStreamName  | Should -Be 'Custom-Defender_AnalyticsData_CL'
    }
}

Describe 'Schema parity · manifest to ARM artifact (per-category-schema)' {
    BeforeAll {
        $script:Artifact = Get-Content (Join-Path $script:RepoRoot 'deploy\per-category-schemas\Defender-AnalyticsData.json') -Raw | ConvertFrom-Json -Depth 50
    }
...
Describe 'Live fan-out + ProjectionMap · GetTopThreats' -Skip {
