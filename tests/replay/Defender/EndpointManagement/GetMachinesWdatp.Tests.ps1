# Per-Op Pester replay test · Defender/EndpointManagement/GetMachinesWdatp
...
BeforeAll {
    ...
    $script:Op = @($script:Manifest.Operations | Where-Object { $_.OperationKey -eq 'GetMachinesWdatp' })[0]
}

Describe 'Manifest contract · Defender/EndpointManagement/GetMachinesWdatp' {
    It 'manifest entry exists for GetMachinesWdatp · no IsActive flag (v11 §4.11)' {
        $script:Op | Should -Not -BeNullOrEmpty
        $script:Op.OperationKey | Should -Be 'GetMachinesWdatp'
...
Describe 'Live fan-out + ProjectionMap · GetMachinesWdatp' -Skip {
