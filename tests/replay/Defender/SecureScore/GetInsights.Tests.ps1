# Per-Op Pester replay test · Defender/SecureScore/GetInsights
...
BeforeAll {
    ...
    $script:Op = @($script:Manifest.Operations | Where-Object { $_.OperationKey -eq 'GetInsights' })[0]
}

Describe 'Manifest contract · Defender/SecureScore/GetInsights' {
    It 'manifest entry exists for GetInsights · no IsActive flag (v11 §4.11)' {
        $script:Op | Should -Not -BeNullOrEmpty
        $script:Op.OperationKey | Should -Be 'GetInsights'
...
Describe 'Live fan-out + ProjectionMap · GetInsights' -Skip {
