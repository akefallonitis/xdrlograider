# Per-Op Pester replay test · Defender/SecureScore/GetSecureScoresV2
...
BeforeAll {
    ...
    $script:Manifest = Import-PowerShellDataFile -Path (Join-Path $script:RepoRoot 'manifests\Defender\SecureScore.psd1') -ErrorAction Stop
    $script:Op = @($script:Manifest.Operations | Where-Object { $_.OperationKey -eq 'GetSecureScoresV2' })[0]
}

Describe 'Manifest contract · Defender/SecureScore/GetSecureScoresV2' {
    It 'manifest entry exists for GetSecureScoresV2 · no IsActive flag (v11 §4.11)' {
        $script:Op | Should -Not -BeNullOrEmpty
        $script:Op.OperationKey | Should -Be 'GetSecureScoresV2'
    ...
    It 'canonical table + stream naming' {
        $script:Op.WorkspaceTable | Should -Be 'Defender_SecureScore_CL'
        $script:Op.DcrStreamName  | Should -Be 'Custom-Defender_SecureScore_CL'
    }
}

Describe 'Schema parity · manifest to ARM artifact (per-category-schema)' {
    BeforeAll {
        $script:Artifact = Get-Content (Join-Path $script:RepoRoot 'deploy\per-category-schemas\Defender-SecureScore.json') -Raw | ConvertFrom-Json -Depth 50
    }
...
Describe 'Live fan-out + ProjectionMap · GetSecureScoresV2' -Skip {
