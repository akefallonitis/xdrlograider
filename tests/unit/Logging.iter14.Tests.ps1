...
Describe 'Logging.SamplingExcludedTypesConfigured' {
    It 'Bicep main.bicep configures APPLICATIONINSIGHTS_TELEMETRY_SAMPLING_EXCLUDED_TYPES with the 3 critical event types' {
...
Describe 'Logging.AppInsightsConnectionStringSet' {
    It 'Bicep configures APPLICATIONINSIGHTS_CONNECTION_STRING (modern; supersedes instrumentation key)' {
        $faBicep = Get-Content -LiteralPath (Join-Path $script:RepoRoot 'deploy' 'modules' 'function-app.bicep') -Raw
