...
Describe 'Manifest Availability schema (declarative contract)' {
    ...
    It 'manifest contains the expected baseline of <ExpectedCount> endpoints (drift detector)' -ForEach @(
        ...
        @{ Description = 'total endpoints (45 baseline + 13 Tier A Phase 2 + 6 Phase 1 - F1 + 2 Phase 2 batches 1-2: PendingActions + IdentityDormantAccounts)'; ExpectedCount = 66; Filter = { $true } }
        @{ Description = 'live endpoints (R++ all-live policy + Phase 2 batches 1-2: 66 - 1 deprecated = 65)'; ExpectedCount = 65; Filter = { $args[0].Availability -eq 'live' } }
