...
Describe 'Manifest Availability schema (declarative contract)' {
    ...
    It 'manifest contains the expected baseline of <ExpectedCount> endpoints (drift detector)' -ForEach @(
        ...
        @{ Description = 'total endpoints (45 baseline + 13 Tier A Phase 2 + 6 Phase 1 - F1 + 5 Phase 2 batches 1-5)'; ExpectedCount = 69; Filter = { $true } }
        @{ Description = 'live endpoints (R++ all-live policy + Phase 2 batches 1-5: 69 - 1 deprecated = 68)'; ExpectedCount = 68; Filter = { $args[0].Availability -eq 'live' } }
