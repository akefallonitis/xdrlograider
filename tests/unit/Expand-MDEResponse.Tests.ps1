...
<#
.SYNOPSIS
    v0.1.0 GA — Expand-MDEResponse fix gate. Verifies the wrapper-key
...
BeforeAll {
    ...
    # The wrapper-key shapes that WERE breaking before v0.1.0 GA
...
Describe 'Expand-MDEResponse — WrapperKey.NoRegression (locked v0.1.0 GA)' {
    ...
    It 'NO row carries EntityId in the forbidden wrapper-key set' {
        ...
        $rows = Expand-MDEResponse -Response $response -UnwrapProperty 'Results'
        foreach ($row in $rows) {
            $script:ForbiddenWrapperKeys | Should -Not -Contain $row.Id -Because "row Id '$($row.Id)' must not be a wrapper-key (pre-v0.1.0 bug regression)"
        }
    }
    ...
    It 'WITHOUT UnwrapProperty, a wrapper-key response would still NOT produce wrapper-key rows when the response is treated as object property-bag (shape 3) — but operators MUST declare UnwrapProperty for paged endpoints' {
        ...
        # v0.1.0 GA doesn't auto-detect — the manifest MUST declare UnwrapProperty.
...
Describe 'Expand-MDEResponse — WrapperKey.ActionCenter (live-fixture replay)' {
    ...
    It 'replays MDE_ActionCenter_CL-raw.json with UnwrapProperty=Results + IdProperty=ActionId; emits per-action rows (NOT 2 wrapper rows)' -Skip:(-not $script:HasActionCenterFixture) {
        ...
        # the v0.1.0 GA fix emits N rows where N = Results.Length.
        ...
        # No row Id is a wrapper-key — locked-in regression gate for pre-v0.1.0 bug
        foreach ($row in $rows) {
...
Describe 'Expand-MDEResponse — ScalarWrap (v0.1.0 GA)' {
...
Describe 'Expand-MDEResponse — Hashtable-vs-PSCustomObject parity (regression: pre-v0.1.0.5)' {
