...
BeforeAll {
    ...
    # ----- DCR-declared columns per stream (from Bicep source) -----
    # Parse dce-dcr.bicep to extract per-stream column declarations. Walk the
    # text and pull every `'Custom-MDE_X_CL': { columns: [ {name: 'A', ...}, ... ] }`
    # block. We look for the stream key + the contained `name: '...'` entries.
    $bicep = Get-Content -Raw -Path $script:DceDcrBicep
    $script:DcrColumnsByStream = @{}

    # Bicep streamDeclarations live inside `streamDeclarations: { ... }`. Per-stream
    # entries look like:
    #   'Custom-MDE_X_CL': {
    #     columns: [
    ...
    #     ]
    #   }
    $blockRegex = "'Custom-(MDE_\w+_CL)':\s*\{[^}]*?columns:\s*\[(.*?)\]"
    foreach ($m in [regex]::Matches($bicep, $blockRegex, 'Singleline')) {
        $stream = $m.Groups[1].Value
        $colsBlob = $m.Groups[2].Value
        $cols = [regex]::Matches($colsBlob, "name:\s*'(\w+)'") | ForEach-Object { $_.Groups[1].Value }
