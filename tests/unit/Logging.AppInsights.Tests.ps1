...
<#
.SYNOPSIS
    v0.1.0 GAB — Microsoft-best-practices structured-logging test
...
.DESCRIPTION
    Test gates by name (referenced in v0.1.0 GAB plan):
...
BeforeAll {
    ...
    # Phase J D'.22 (2026-05-04): AppInsights helpers extracted from
    # Xdr.Sentinel.Ingest into Xdr.Common.Telemetry.
    $script:TelemetryPath = Join-Path $script:RepoRoot 'src' 'Modules' 'Xdr.Common.Telemetry' 'Xdr.Common.Telemetry.psd1'
    Import-Module $script:TelemetryPath -Force -ErrorAction Stop
...
Describe 'Send-XdrAppInsights* module surface' {
    It 'exports all 4 Send-XdrAppInsights* entry points (from Xdr.Common.Telemetry per Phase J D''.22)' {
        $exported = (Get-Module Xdr.Common.Telemetry).ExportedFunctions.Keys
...
Describe 'Logging.NoSecretsLeaked' {
    ...
    It 'does not mutate the caller hashtable' {
        ...
        $null = & (Get-Module Xdr.Common.Telemetry) {
...
Describe 'Logging.AuthChainEventsStructured' {
    ...
    # v0.1.0 GA (v0.1.0 GA): Stream.Polled migrated from customEvent to
    ...
    # v0.1.0 GA (v0.1.0 GA): AuthChain.AADSTSError migrated from customEvent
...
Describe 'Logging.NoPlainAADSTSMessages' {
    # v0.1.0 GA (v0.1.0 GA): AADSTS errors now flow to AppExceptions
