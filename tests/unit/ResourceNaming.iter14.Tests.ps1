...
BeforeDiscovery {
    ...
    # .internal/bicep-reference/ in v0.1.0-beta first publish — ARM is SoT).
    $script:DiscoveryRepoRoot   = (Resolve-Path (Join-Path $PSScriptRoot '..' '..')).Path
    $script:DiscoveryBicepSource = Join-Path $script:DiscoveryRepoRoot 'deploy' 'main.bicep'
}
...
Describe 'ResourceNaming.LegacyEnvInName.Param' {
    ...
    It 'legacyEnvInName defaults to false (env-as-tag is the canonical Azure Well-Architected pattern)' {
        $script:ArmJson.parameters.legacyEnvInName.defaultValue | Should -Be $false -Because 'default flipped to false in v0.2.0 — env signal carried via tag, not via resource-name infix. Operators on legacy `xdrlr-prod-*` deploys must opt-in to true explicitly.'
    }

    It 'main.bicep source declares legacyEnvInName parameter with default false' -Skip:(-not (Test-Path -LiteralPath $script:DiscoveryBicepSource)) {
        # Bicep is archived to .internal/bicep-reference/ in v0.2.0 (ARM is the
        # single source of truth). Skip cleanly when not present in the working tree.
        $bicepSource = Get-Content -LiteralPath $script:BicepSource -Raw
        $bicepSource | Should -Match 'param\s+legacyEnvInName\s+bool\s*=\s*false' -Because 'Bicep source must mirror compiled ARM (default false)'
    }
}
...
Describe 'ResourceNaming.LegacyOptIn.NameCompositionShape' {
    # Shape gate: regardless of the parameter default, the name-composition
    # expressions must thread through envSegment / stEnvSegment so an operator
    # who flips legacyEnvInName=true (in-place upgrade from a pre-v0.2.0
    # `xdrlr-prod-*` deploy) gets identical names to iter-13.15.
    # When legacyEnvInName=true:  xdrlr-prod-fn-<suffix>, xdrlrprodst<suffix>, etc.
    # When legacyEnvInName=false: xdrlr-fn-<suffix>,      xdrlrst<suffix>,      etc. (default)

    It 'funcName expression composes via envSegment + "fn-" infix (operator-toggleable env embedding)' {
        # Reduces to: concat(projectPrefix, '-', envSegment, 'fn-', suffix)
        # Both legacy and env-as-tag forms reach the right name through the
        # envSegment toggle — assert the expression *shape*, not the runtime value.
    ...
    It 'stName expression composes via stEnvSegment + "st" infix (toLower-wrapped, hyphen-stripped)' {
        # Reduces to: toLower(replace(concat(projectPrefix, stEnvSegment, 'st', suffix), '-', ''))
        # Storage account names disallow hyphens → stEnvSegment is a separate
        # variant of envSegment without the trailing dash.
