...
Describe 'Manifest.ProjectionMap.Populated' {
    ...
    It 'all 67 non-deprecated streams have populated ProjectionMap (v0.1.0 GA F1 + Phase 2 batches 1-4: 68 - 1 deprecated = 67 live)' {
        $populated = 0
        foreach ($stream in $script:Manifest.Keys) {
            $entry = $script:Manifest[$stream]
            if ($entry.Availability -eq 'deprecated') { continue }
        ...
        $populated | Should -Be 67 -Because 'every non-deprecated stream populates ProjectionMap for typed-column ingest (68 - 1 deprecated; Phase 2 batches 1-4 added PendingActions + IdentityDormantAccounts + IdentityLateralMovementPaths + VulnerabilityCertificates 2026-05-09)'
    }
}
