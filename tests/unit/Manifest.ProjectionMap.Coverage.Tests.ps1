...
Describe 'Manifest.ProjectionMap.Populated' {
    ...
    It 'all 66 non-deprecated streams have populated ProjectionMap (v0.1.0 GA F1 + Phase 2 batches 1-3: 67 - 1 deprecated = 66 live)' {
        $populated = 0
        foreach ($stream in $script:Manifest.Keys) {
            $entry = $script:Manifest[$stream]
            if ($entry.Availability -eq 'deprecated') { continue }
        ...
        $populated | Should -Be 66 -Because 'every non-deprecated stream populates ProjectionMap for typed-column ingest (67 - 1 deprecated; Phase 2 batches 1-3 added PendingActions + IdentityDormantAccounts + IdentityLateralMovementPaths 2026-05-09)'
    }
}
