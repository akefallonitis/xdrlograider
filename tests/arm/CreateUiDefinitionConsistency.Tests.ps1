...
Describe 'Dropdowns.ConsumptionY1Tier' {

    It 'wizard hostingPlan dropdown emits canonical 3-tier values matching ARM' {
        # Belt-and-suspenders gate against the v0.1.0-beta first-publish bug
        # class: the wizard MUST emit the same 3-tier semantic enum the ARM
        # accepts (consumption-y1 / flex-fc1 / premium-ep1). If a future edit
        # accidentally renames one side, this gate fires before the deploy
        # validator does.
        $script:UiInputElements.ContainsKey('hostingPlan') | Should -BeTrue -Because 'wizard must expose the hostingPlan dropdown to operators'
        $el = $script:UiInputElements['hostingPlan']
        $el.AllowedValues | Should -Contain 'consumption-y1'
        $el.AllowedValues | Should -Contain 'flex-fc1'
        $el.AllowedValues | Should -Contain 'premium-ep1'
        $script:ArmParams.ContainsKey('hostingPlan') | Should -BeTrue
        $hostingPlanParam = $script:ArmParams['hostingPlan']
        ($hostingPlanParam.PSObject.Properties.Name -contains 'allowedValues') | Should -BeTrue -Because 'ARM hostingPlan parameter must declare allowedValues — otherwise the operator could pass an unsupported tier'
        $hostingPlanParam.allowedValues | Should -Contain 'consumption-y1'
        $script:ArmParams['hostingPlan'].allowedValues | Should -Contain 'flex-fc1'
        $script:ArmParams['hostingPlan'].allowedValues | Should -Contain 'premium-ep1'
    }
}
