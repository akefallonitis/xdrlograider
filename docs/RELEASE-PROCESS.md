...
- [ ] **All offline tests pass** — `pwsh ./tests/Run-Tests.ps1 -Category all-offline` returns `307 passed` (or more as tests grow)
...
- [ ] **Bicep source matches compiled ARM** — `deploy/compiled/mainTemplate.json` reflects current `deploy/main.bicep` intent
...
4. **`.github/workflows/release.yml` fires automatically** on the tag push:
   ...
   - Compiles Bicep → scratch (syntax check; does NOT overwrite `deploy/compiled/mainTemplate.json`)
