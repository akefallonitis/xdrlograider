...
**Until then**: the Solution ZIP is built and available as `xdrlograider-solution-0.1.0-beta.zip` in each GitHub release. You can side-load it via:

```bash
# Download the Solution ZIP from GitHub release
curl -LO 'https://github.com/akefallonitis/xdrlograider/releases/download/v0.1.0-beta/xdrlograider-solution-0.1.0-beta.zip'

# Extract + deploy mainTemplate.json from within — same as Method 2
unzip xdrlograider-solution-0.1.0-beta.zip -d xdrlr-solution
az deployment group create \
    --resource-group xdrlr-prod-rg \
...
```
WEBSITE_RUN_FROM_PACKAGE=https://github.com/akefallonitis/xdrlograider/releases/download/v0.1.0-beta/function-app.zip
```
