...
> Threat model, per-hosting-plan residual risk, supply-chain verification, and Marketplace v1.2 hardening roadmap.
> Living document — updated each release window.

## Supply chain verification (Sigstore-cosign keyless)

All v0.1.0 GA release artifacts are signed via Sigstore-cosign keyless signing
(GitHub Actions OIDC token → Fulcio cert → Rekor transparency log). 6 artifacts
each ship with `.sig` (signature) + `.bundle` (verification material) companions:

| Artifact | Sig file | Bundle file |
|---|---|---|
| function-app.zip | function-app.zip.sig | function-app.zip.bundle |
| mainTemplate.json | mainTemplate.json.sig | mainTemplate.json.bundle |
| sentinelContent.json | sentinelContent.json.sig | sentinelContent.json.bundle |
| createUiDefinition.json | createUiDefinition.json.sig | createUiDefinition.json.bundle |
| xdrlograider-solution-{version}.zip | (same).sig | (same).bundle |
| xdrlograider-sbom.spdx.json | (same).sig | (same).bundle |

### Operator verification command

For each downloaded artifact, run:

```bash
cosign verify-blob \
    --certificate-identity-regexp 'akefallonitis' \
...
Expected output: `Verified OK`. Any other output = signature invalid; do NOT deploy.

The `--certificate-identity-regexp 'akefallonitis'` matches the GitHub repository
owner identity that signed the artifact. The `--certificate-oidc-issuer` confirms
the signature was issued by GitHub Actions (not a local cosign-key signature).

### Why keyless signing

Per Sigstore best practice (https://docs.sigstore.dev/cosign/keyless), keyless
signing using GitHub Actions OIDC tokens removes operator key-management burden
while preserving full chain-of-custody via the Sigstore Rekor transparency log.
Every signature is publicly verifiable + tamper-evident.

### SBOM SPDX

`xdrlograider-sbom.spdx.json` contains a Software Bill of Materials in SPDX 2.3
format generated by `anchore/sbom-action@v0.24.0`. Operators with regulatory
SBOM requirements can ingest into their preferred SBOM management tool.
