...
try {
    ...
    if ($null -eq $kvs -or $kvs.Count -eq 0) {
        ...
        # v0.1.0-beta-fix: data-plane is the AUTHORITATIVE source for secret
        # listing. The previous management-plane fallback caused FALSE NEGATIVES
        # (eventual-consistency reads returned partial lists, e.g., 3-of-4 when
        # all 4 were genuinely present). When the audit-SP lacks data-plane
        # RBAC, FAIL LOUDLY with the exact remediation command — never silently
        # fall back to an unreliable management-plane query.
        $secretNames    = @()
        $dataPlaneError = $null
        try {
            ...
            $dataPlaneError = $_.Exception.Message
        }

        if ($dataPlaneError) {
            $remediation = @(
                "Audit-SP $($env:AZURE_CLIENT_ID) needs 'Key Vault Secrets User' on the KV.",
                "Grant via: az role assignment create --role 'Key Vault Secrets User'",
                "  --assignee $($env:AZURE_CLIENT_ID) --scope $($kv.ResourceId)",
                "Then re-run this script (RBAC propagation: 1-3 min).",
                "Original error: $dataPlaneError"
            ) -join ' '
            Record-Phase 'P3.5' 'Key Vault structure' $false "DATA-PLANE DENIED — cannot reliably enumerate secrets via management plane (eventual-consistency false negatives). $remediation" ''
            # Skip the rest of P3.5 — without authoritative secret list, every
            # downstream check would be guessing. (Cannot `return` here — this
            # block is at script top level, not in a function.)
        } else {
            # v0.1.0-beta uses 4 separate per-field secrets (NOT a single
            # 'mde-portal-auth' JSON blob): mde-portal-{upn,password,totp,auth-method}.
...
try {
    $rows = Invoke-WorkspaceKql -Query 'XdrConnectorHealth_CL | where TimeGenerated > ago(15m) | where StreamsSucceeded > 0 | top 1 by TimeGenerated desc | project TimeGenerated, Tier, FunctionName, StreamsSucceeded, StreamsAttempted'
    if (@($rows).Count -eq 0) {
        Record-Phase 'P4' 'Heartbeat liveness' $false 'No XdrConnectorHealth_CL rows with StreamsSucceeded > 0 in last 15 min — wait 5-10+ min after deploy or check AuthChain.* events in App Insights' ''
    } else {
        $r = $rows[0]
...
try {
    $rows = Invoke-WorkspaceKql -Query 'XdrConnectorHealth_CL | where TimeGenerated > ago(2h) | summarize Bins = count() by bin(TimeGenerated, 5m) | summarize TotalBins = count()'
...
# P6. Rate limits = 0
try {
    $rows = Invoke-WorkspaceKql -Query 'XdrConnectorHealth_CL | where TimeGenerated > ago(2h) | summarize TotalRate429 = sum(coalesce(toint(Rate429Count), 0))'
...
try {
    $rows = Invoke-WorkspaceKql -Query 'XdrConnectorHealth_CL | where TimeGenerated > ago(2h) and isnotnull(GzipBytes) and isnotnull(RowsIngested) and RowsIngested > 0 | summarize avg(todouble(GzipBytes) / todouble(RowsIngested))'
...
try {
    ...
    if ($faName) {
        ...
        if (-not $principalId) {
            Record-Phase 'P12' 'SAMI 7-role check' $false "FA=$faName has no Managed Identity assigned (cold start may not have completed)" ''
        } else {
            # v0.1.0-beta Y1 baseline = 7 role assignments:
            ...
            # The check counts each role NAME irrespective of scope; 5 MMP rows
            # collapse to the same role name so we expect at least 1 of each role
            # name AND a total >= 7 assignments.
            $assignments     = @(Get-AzRoleAssignment -ObjectId $principalId -ErrorAction SilentlyContinue)
            $expectedRoles   = @('Key Vault Secrets User','Storage Table Data Contributor','Monitoring Metrics Publisher')
            $found           = @($assignments | Where-Object { $expectedRoles -contains $_.RoleDefinitionName })
            $foundRoleNames  = @($found | ForEach-Object { $_.RoleDefinitionName } | Sort-Object -Unique)
            $missingRoleNames = @($expectedRoles | Where-Object { $foundRoleNames -notcontains $_ })
            $mmpCount        = @($found | Where-Object { $_.RoleDefinitionName -eq 'Monitoring Metrics Publisher' }).Count
            $kvOk            = $foundRoleNames -contains 'Key Vault Secrets User'
            $stOk            = $foundRoleNames -contains 'Storage Table Data Contributor'
            $verdict         = $kvOk -and $stOk -and ($mmpCount -ge 5) -and ($found.Count -ge 7)
            $detail          = "FA=$faName Principal=$principalId  Found $($found.Count)/7 (KV=$kvOk Storage=$stOk MMP=$mmpCount/5)"
            $missingDetail   = if ($missingRoleNames.Count -gt 0) { " Missing role names: $($missingRoleNames -join ',')" } else { '' }
            Record-Phase 'P12' 'SAMI 7-role check' $verdict "$detail$missingDetail" ('Roles: ' + (($found | ForEach-Object { $_.RoleDefinitionName }) -join ', '))
        }
    } else {
        Record-Phase 'P12' 'SAMI 7-role check' $false 'No Function App found in connector RG' ''
    }
} catch { Record-Phase 'P12' 'SAMI 7-role check' $false "$_" '' }
