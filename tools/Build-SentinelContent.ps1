...
$parsers = @()
# Sort by BaseName for deterministic output across Windows / Linux runners.
# Without this, Get-ChildItem order differs and CI recompile-gate diffs.
foreach ($file in (Get-ChildItem -Path $parsersDir -Filter '*.kql' | Sort-Object BaseName)) {
    $name = $file.BaseName
...
$workbooks = @()
foreach ($file in (Get-ChildItem -Path $workbooksDir -Filter '*.json' | Sort-Object BaseName)) {
    $name     = $file.BaseName
...
$rules = @()
foreach ($file in (Get-ChildItem -Path $rulesDir -Filter '*.yaml' | Sort-Object BaseName)) {
...
$huntingQueries = @()
foreach ($file in (Get-ChildItem -Path $huntingDir -Filter '*.yaml' | Sort-Object BaseName)) {
    $yaml = Get-Content $file.FullName -Raw
    $parsed = ConvertFrom-YamlSimple -Yaml $yaml
