...
try {
    ...
    # 5b — All 21 analytic rules have enabled: false (14 detection + 7 XdrOps per v0.1.0 GA)
    ...
    if ($rules.Count -eq 21 -and $missingEnabled.Count -eq 0) {
        Add-Check -Section '5-Hub' -Name 'All 21 rules ship enabled:false' -Status Pass -Detail '21/21'
    } else {
        Add-Check -Section '5-Hub' -Name 'All 21 rules ship enabled:false' -Status Fail -Detail "rules=$($rules.Count) missing-enabled=$($missingEnabled.Count)"
    }
    ...
    if ($hunts.Count -eq 12 -and $missingMeta.Count -eq 0) {
        Add-Check -Section '5-Hub' -Name 'All 12 hunting queries have metadata' -Status Pass -Detail '12/12'
    } else {
...
try {
    ...
    # CRITICAL — packageUrl liveness probe.
    # mainTemplate.json variables.packageUrl is an ARM expression like
    # [concat('https://github.com/', parameters('githubRepo'),
    #         '/releases/latest/download/function-app.zip')]
    # We resolve parameters('githubRepo') to its declared defaultValue and
    # probe the resulting URL with HEAD.
    #
    # GitHub's /latest endpoint SKIPS pre-release tags (returns 404). If the
    # most-recent release is marked prerelease=true on GitHub, the FA's
    # WEBSITE_RUN_FROM_PACKAGE will 404 on cold start → silent failure
    # (FA shows Running, but no timer ever fires, no telemetry ever emits).
    # Bug class hit live in v0.1.0-beta first deploy; gate prevents recurrence.
    $pkgUrlExpr = [string]$arm.variables.packageUrl
    $resolvedUrl = $pkgUrlExpr
    foreach ($paramName in $arm.parameters.PSObject.Properties.Name) {
        $defaultVal = $arm.parameters.$paramName.defaultValue
        if ($null -ne $defaultVal -and $defaultVal -is [string]) {
            # ARM-style parameters('X') reference → substitute string literal.
            $pattern = "parameters\(\s*'$([regex]::Escape($paramName))'\s*\)"
            $resolvedUrl = [regex]::Replace($resolvedUrl, $pattern, "'$defaultVal'")
        }
    }
    # Strip the [concat(..., ..., ...)] wrapper and concatenate string literals.
    if ($resolvedUrl -match "^\[concat\((.+)\)\]$") {
        $parts = [regex]::Matches($Matches[1], "'([^']*)'") | ForEach-Object { $_.Groups[1].Value }
        $resolvedUrl = ($parts -join '')
    }
    if ($resolvedUrl -match '^https?://') {
        try {
            $head = Invoke-WebRequest -Uri $resolvedUrl -Method Head -MaximumRedirection 5 -ErrorAction Stop -SkipHttpErrorCheck
            $code = [int]$head.StatusCode
            if ($code -ge 200 -and $code -lt 400) {
                Add-Check -Section '8-Flow' -Name 'packageUrl resolves (200/3xx)' -Status Pass -Detail "$resolvedUrl -> HTTP $code"
            } elseif ($code -eq 404) {
                Add-Check -Section '8-Flow' -Name 'packageUrl resolves' -Status Fail -Detail "$resolvedUrl returned 404 — likely the most-recent GitHub release is marked prerelease=true (which /latest skips). Toggle off via: gh release edit <tag> --prerelease=false. release.yml hardened to set prerelease=false in v0.1.0-beta. Defended by tests/arm/ReleasePackageUrlInvariants.Tests.ps1."
            } else {
                Add-Check -Section '8-Flow' -Name 'packageUrl resolves' -Status Fail -Detail "$resolvedUrl returned HTTP $code"
            }
        } catch {
            Add-Check -Section '8-Flow' -Name 'packageUrl resolves' -Status Warn -Detail "could not probe ${resolvedUrl} - $($_.Exception.Message) (offline preflight; rerun with network)"
        }
    } else {
        Add-Check -Section '8-Flow' -Name 'packageUrl resolves' -Status Fail -Detail "could not resolve packageUrl ARM expression to a URL: ${pkgUrlExpr}"
    }
} catch {
