...
| Content | 10 workbooks · 21 analytic rules (14 detection + 7 XdrOps incl. RowVolumeSpike cost-budget gate + StreamWentDry per-stream stale alert) · 12 hunting queries · 4 KQL drift parsers + 11 consolidated LA tables (10 `Defender_<Category>_CL` + 1 `XdrConnectorHealth_CL`) · 360+ sample queries (5 per active stream × 72) — all auto-deployed via nested ARM. Every parser / rule / query / workbook column reference verified against live fixtures in CI. |
| License | MIT |

XdrLogRaider ingests the tenant-configuration surface that Microsoft's first-party APIs don't expose: suppression rule changes, exclusion list changes, data export destination adds, Live Response policy relaxations, XSPM attack paths + chokepoints + top targets + asset classification schema, posture metrics + secure-score per-category, attack-surface analytical paths/chokepoints, threat-analytics enriched outbreaks + top threats, MDI identity service accounts, Action Center approval history, and more. **Drift happens on the KQL side** (pure query-time) — 4 cadence-tier parsers (`MDE_Drift_Configuration` / `MDE_Drift_Inventory` / `MDE_Drift_Exposure` / `MDE_Drift_Maintenance`) feed 10 workbooks (incl. MDE_DeviceInventory_Unified per-device 360° drilldown + XdrLogRaider_ConnectorOps operator dashboard) and 21 analytic rules (16/21 with `entityMappings` populated for Sentinel investigation graph drilldown). `RawJson` is preserved on every row as failover/forensic field; typed columns projected at ingest via DCR (Architecture J canonical Sentinel Entity Type cols across 7 high-value streams). Every endpoint response shape is captured as a live fixture in `tests/fixtures/live-responses/` and all parsers + rules + queries + workbooks are verified against those fixtures in CI (2222+ Pester tests across 118 files, plus 12-edge wiring audit per stream + per-tier drift cardinality regression-locker).
...
- Adds 11 custom tables (10 `Defender_<Category>_CL` consolidated category tables + 1 `XdrConnectorHealth_CL` ops table) + a Sentinel Data Connector UI card with 360+ sample queries + 4 drift parsers / 10 workbooks / 21 analytic rules / 12 hunting queries to your existing workspace (via cross-RG nested deployments — no manual Sentinel-content install)
...
|---|---|---|
| **v0.1.0 GA** ← *current* | Pure Defender connector | 72 portal-only streams (71 live + 1 deprecated) · 11 consolidated LA tables · **13 per-category DCRs** (semantic split for oversized cats: `xdrlr-dcr-actioncenter`, `xdrlr-dcr-config-alerts-detection`, `xdrlr-dcr-config-platform-rbac`, `xdrlr-dcr-exposure-attack-surface`, `xdrlr-dcr-exposure-posture-score`, etc.) · 1 DCE · 10 workbooks (incl. MDE_DeviceInventory_Unified + XdrLogRaider_ConnectorOps) · 21 analytic rules (14 detection + 7 XdrOps; 16/21 with populated entityMappings) · 12 hunting queries · 4 cadence-tier drift parsers · 4 schema-consistency CI gates · cosign-signed release artifacts · Architecture J canonical Sentinel Entity Type cols across all streams · `tools/Audit-DcrSchema.ps1` + `tools/Build-DcrSection.ps1` + `tools/Verify-CosignArtifacts.ps1` |
