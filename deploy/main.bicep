...
//
// Topology (v1.0): the customer ALREADY HAS a Sentinel-enabled Log Analytics workspace.
// This template does NOT create or modify the workspace itself. It deploys:
//   - Connector resources in the target RG (Function App + plan + KV + Storage + App Insights + DCE + DCR)
//   - 54 custom tables as sub-resources INSIDE the customer's workspace (cross-RG if needed)
//   - A Sentinel Data Connector UI card inside the workspace
//
...
// Sentinel Data Connector UI card: also a workspace sub-resource (solutions/dataConnectors).
// Deploys at the workspace's RG scope.
