...
- **portal_endpoint_broken** — a currently-shipped stream stops returning
  rows; include the CSV + markdown from `tests/integration/Audit-Endpoints-Live.ps1`
...
|------------|--------|---------------------|
| 10+ of 64 streams return 4xx on tenants without the relevant Microsoft feature/license (MDI, TVM add-on, MTO, MCAS, etc.) | Expected behaviour — classified `tenant-feature-gated` in the manifest at runtime via SuccessKind side-channel. No rows until customer provisions the feature. | Auto-activates when customer enables feature; no code change needed. |
